...
Format the output using clean, simple HTML tags.
- Use <strong> for bolding.
- Use <ul> and <li> for bulleted lists.
- Use <ol> and <li> for numbered lists.
- Do NOT use inline styles (e.g., style="...").
- Do NOT include any markdown characters like '*' in the final HTML output.
- Do NOT include any placeholder text like '[Contact Information]'.

The output should be only the HTML, ready to be rendered.
...
"""
    ...
    summary_output = summary_response["response"].strip()
    # Remove markdown code block fences if present
    if summary_output.startswith("```html"):
        summary_output = summary_output[len("```html"):].strip()
    if summary_output.endswith("```"):
        summary_output = summary_output[:-len("```")].strip()
