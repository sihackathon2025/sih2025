# Fake Data Generation Script for Admindashboard

This script can be used to populate your Django database with fake data for `Village`, `HealthReport`, `ClinicReport`, and `NgoSurvey` models. This will help in testing the `admindashboard` functionality.

**Instructions:**

1.  **Start your Django shell:**
    ```bash
    python manage.py shell
    ```
2.  **Copy and paste the entire Python script below** into your Django shell and press Enter.
3.  Wait for the script to finish executing.

After the script completes, you can then run `from admindashboard.services import aggregate_all_villages; aggregate_all_villages()` again to see the aggregated data.
...
fake_ngo_user, _ = User.objects.get_or_create(username='fake_ngo', defaults={'email': 'ngo@example.com', 'password': 'pbkdf2_sha256$600000$y0urh4shh3r3$y0urh4shh3r3'})
fake_asha_user, _ = User.objects.get_or_create(username='fake_asha', defaults={'email': 'asha@example.com', 'password': 'pbkdf2_sha256$600000$y0urh4shh3r3$y0urh4shh3r3'})
...
for village in villages:
    ...
    for _ in range(random.randint(5, 15)): # 5 to 15 reports per village
        ...
        HealthReport.objects.create(
            ...
            asha_worker_id=fake_asha_user.id, # Assuming ASHA worker ID is user ID
            state=village.state,
