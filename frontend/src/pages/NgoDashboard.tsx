...
interface NgoReport {
  id?: number; // Optional, as it's auto-generated by the backend
  ngo_id: number;
  village_id: number;
...
const NgoDashboard = () => {
  ...
  const { data: villages, isLoading: isLoadingVillages, error: villagesError } = useQuery<Village[]>({ 
    queryKey: ['villages'], 
    queryFn: async () => {
  ...
  const { data: ngoReports, isLoading: isLoadingNgoReports, error: ngoReportsError } = useQuery<NgoReport[]>({ 
    queryKey: ['ngoReports'], 
    queryFn: async () => {
      const response = await api.get('/data/ngo-surveys/');
      return response.data;
    },
  });

  if (isLoadingVillages || isLoadingNgoReports) {
    return <div>Loading dashboard data...</div>;
  }

  if (villagesError || ngoReportsError) {
    return <div>Error loading data: {villagesError?.message || ngoReportsError?.message}</div>;
  }

  if (!villages || !ngoReports) {
  ...
  const handleSubmitReport = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/data/ngo-surveys/', report);
      toast.success('Survey data added successfully!');
      setShowAddData(false);
      // Optionally, refetch ngoReports to update the list
      // queryClient.invalidateQueries(['ngoReports']);
    } catch (error) {
      console.error('Error submitting NGO report:', error);
